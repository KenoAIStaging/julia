...
function codeinfo_to_ir(ci::Core.CodeInfo; nargs::Int, name::Symbol = :f)
    ...
    function convert_expr!(i::Int, st::Expr)
        ...
        elseif h === :aliasscope || h === :popaliasscope
            # region brackets consumed by codegen's linear alias-scope stack:
            # real statements, not metadata (dropping them silently loses the
            # `@aliasscope`/`Const` noalias annotations)
