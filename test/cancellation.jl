...
@testset "^C" begin
    ...
    # TODO(port): two further ^C subprocess tests are deferred while the port
    # of #60281 proceeds: the @sync compute-spinner test needs signal-thread
    # episode marking + bound-source propagation (the -t1 child's listener task
    # starves behind the compiled spinner), and the escalation test needs the
    # rescue-timer ladder and task abandonment. They are restored by the
    # commits that port that machinery.
end
