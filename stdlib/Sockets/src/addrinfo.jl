...
function uv_getaddrinfocb(req::Ptr{Cvoid}, status::Cint, addrinfo::Ptr{Cvoid})
    ...
    w = unsafe_pointer_to_objref(data)::Base.WaitEntry
    t = w.task
    if !(t isa Task && Base.claim_wait(t, w))
...
function _wait_dns_req(req::Ptr{Cvoid}, tok::Base.MaybeToken)
    ...
    w = Base._cached_wait_entry(ct)
    Base._arm_wait(ct, w)
    # There is no waitee object for a DNS request; record the request itself
    # as the queue witness, so the entry counts as in-use (and is not reused
    # for a nested park, e.g. on the contended iolock during unwind) until
    # the callback or the teardown below releases it.
    w.queue = req
    preserve_handle(ct)
