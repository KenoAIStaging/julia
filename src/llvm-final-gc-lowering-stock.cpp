...
void FinalLowerGC::lowerWriteBarrier(CallInst *target, Function &F) {
    ...
    auto parInImage = builder.CreateAnd(parTag, ConstantInt::get(T_size, GC_IN_IMAGE | GC_IN_IMAGE_MUTATED), "parent_in_image");
    auto parIsImage = builder.CreateICmpEQ(parInImage, ConstantInt::get(T_size, GC_IN_IMAGE_NOT_MUTATED), "parent_is_image");
