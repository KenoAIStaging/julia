...
static void jl_write_values(jl_serializer_state *s) JL_CANSAFEPOINT JL_GC_DISABLED
{
    ...
    for (size_t item = 0; item < l; item++) {
        ...
        if (jl_is_array(v)) {
            ...
            write_pointerfield(s, jl_nothing); // waiters_tail (transient; reset)
            write_pointerfield(s, jl_nothing); // watchers (transient; reset)
            write_uint8(f, jl_atomic_load_relaxed(&cs->state));
            write_uint8(f, 0);                 // delivered (transient episode state)
            write_uint8(f, 0);                 // _lock
            write_padding(f, offsetof(jl_cancel_source_t, nparents) - 4 * sizeof(void*) - 3 * sizeof(uint8_t));
