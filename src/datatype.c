...
static jl_tvar_t *find_typegroup_ref(jl_value_t *t, htable_t *subst_map) JL_NOTSAFEPOINT
{
    ...
    if (jl_is_unionall(t)) {
        jl_unionall_t *ua = (jl_unionall_t*)t;
        jl_tvar_t *found = find_typegroup_ref(ua->lb, subst_map);
        if (!found)
            found = find_typegroup_ref(ua->ub, subst_map);
...
static jl_value_t *resolve_type_refs(jl_value_t *t, htable_t *subst_map, jl_deferred_typecache_t *dcache)
{
    ...
    if (jl_is_unionall(t)) {
        ...
        lb = resolve_type_refs(ua->lb, subst_map, dcache);
        ub = resolve_type_refs(ua->ub, subst_map, dcache);
    ...
    if (jl_is_vararg(t)) {
        ...
        JL_GC_PUSH2(&T, &N);
        T = vm->T ? resolve_type_refs(vm->T, subst_map, dcache) : NULL;
...
static int type_references_any(jl_value_t *t, htable_t *set) JL_NOTSAFEPOINT
{
    ...
    if (jl_is_unionall(t)) {
        jl_unionall_t *ua = (jl_unionall_t*)t;
        return type_references_any(ua->lb, set) ||
               type_references_any(ua->ub, set) ||
