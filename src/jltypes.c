...
JL_DLLEXPORT jl_value_t *jl_type_unionall(jl_tvar_t *v, jl_value_t *body)
{
    ...
    jl_value_t *newbody = inst_type_w_(body, &env, NULL, 0, 0, NULL);
    JL_GC_PUSH1(&newbody);
...
{
    jl_typeenv_t env = { NULL, p, NULL };
    return inst_type_w_(u->body, &env, NULL, 1, nothrow, NULL);
}
...
JL_DLLEXPORT jl_value_t *jl_unionall_open(jl_unionall_t *u, jl_tvar_t **vout)
{
    ...
    jl_value_t *body = inst_type_w_(u->body, &env, NULL, 1, 2, NULL);
    if (body == NULL)
...
JL_DLLEXPORT jl_value_t *jl_unionall_open2(jl_unionall_t *u) JL_CANSAFEPOINT
{
    jl_tvar_t *v = NULL;
...
static jl_value_t *substitute_tvarref(jl_value_t *t, size_t idx, jl_value_t *val, int nothrow) JL_CANSAFEPOINT
{
    assert(idx > 0);
    ...
    return inst_type_w_(t, &env[0], NULL, 1, nothrow, NULL);
}
...
static jl_value_t *shift_refs_(jl_value_t *t, ssize_t inc, size_t depth) JL_CANSAFEPOINT
{
    if (jl_is_tvarref(t)) {
    ...
    if (jl_is_datatype(t) && ((jl_datatype_t*)t)->hasescapingrefs) {
        ...
        if (bound) {
            ...
            if (primary != NULL && tn != jl_tuple_typename &&
                ...
                t = inst_datatype_inner(primary, NULL, iparams, ntp, NULL, &penv[ntp - 1], 0, 0, NULL);
            }
            else {
                t = inst_datatype_inner(dt, NULL, iparams, ntp, NULL, NULL, 0, 0, NULL);
            }
        }
        JL_GC_POP();
        return t;
...
static int type_references_deferred(jl_value_t *t, jl_deferred_typecache_t *dcache) JL_NOTSAFEPOINT
{
    ...
    if (jl_is_unionall(t)) {
        jl_unionall_t *ua = (jl_unionall_t*)t;
        return type_references_deferred(ua->lb, dcache) ||
               type_references_deferred(ua->ub, dcache) ||
                                       ...
                                       jl_deferred_typecache_t *dcache)
{
    ...
    if (istuple || isnamedtuple) {
        ...
        jl_value_t *super = inst_type_w_((jl_value_t*)dt->super, env, &stop, check, nothrow, dcache);
    ...
    if (defer_selfref) {
        ...
        if (ftypes == jl_emptysvec) {
            ...
            if (dt->types == NULL)
                jl_gc_write(ndt, ndt->types, jl_svec_t, compute_fieldtypes_(ndt, &stop, cacheable && !defer, dcache));
            else
                jl_gc_write(ndt, ndt->types, jl_svec_t, inst_ftypes(ftypes, env, &stop, cacheable && !defer, dcache));
        }
    }
...
static jl_value_t *inst_type_w_(jl_value_t *t, jl_typeenv_t *env, jl_typestack_t *stack, int check, int nothrow, jl_deferred_typecache_t *dcache)
{
    ...
    if (jl_is_typeapp(t)) {
        ...
        jl_value_t *h = NULL, *p = NULL;
        JL_GC_PUSH2(&h, &p);
        h = inst_type_w_(ta->head, env, stack, check, nothrow ? 1 : 0, dcache);
        if (h != NULL)
            p = inst_type_w_(ta->param, env, stack, check, nothrow ? 1 : 0, dcache);
        if (h == NULL || p == NULL) {
            assert(nothrow);
            t = NULL;
        }
    ...
    if (jl_is_unionall(t)) {
        ...
        lb = inst_type_w_(ua->lb, env, stack, check, nothrow ? 1 : 0, dcache);
        if (lb == NULL) {
            assert(nothrow);
            t = NULL;
        }
        if (t != NULL) {
            ub = inst_type_w_(ua->ub, env, stack, check, nothrow, dcache);
            if (ub == NULL) {
