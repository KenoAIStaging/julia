...
remap_refs!(col, rs) = hasrefs(typeof(col)) ?
    error(LazyString("column ", typeof(col), " declares hasrefs but does not implement remap_refs!")) : col
...
SparseCol{T}() where {T} = SparseCol{T}(IdDict{Int32,T}())
...
mutable struct DictColumns
    cols::IdDict{Symbol,SparseCol{Any}}
end
DictColumns() = DictColumns(IdDict{Symbol,SparseCol{Any}}())
...
function col_compact!(c::SparseCol{T}, old_of_new::Vector{Int32}) where {T}
    new_of_old = IdDict{Int32,Int32}(old_of_new[i] => Int32(i) for i in 1:length(old_of_new))
    nd = IdDict{Int32,T}()
    for (k, v) in c.data
...
stage_copy_cols(d::DictColumns) =
    DictColumns(IdDict{Symbol,SparseCol{Any}}(k => stage_copy_col(v) for (k, v) in d.cols))
    ...
    if src isa NamedTuple
        for name in keys(src)
            name in keys(target_cols) || name in drop ||
                error(LazyString("convert_universe: source column :", name, " missing from target universe; list it in `drop` to drop explicitly"))
        end
    end
    n = Int(ir.body.len)
...
copy_col!(t, s, n) = error(LazyString("no column copy defined between ", typeof(s), " and ", typeof(t)))
...
mutable struct ProvenanceCol
    data::IdDict{Int32,Any}
end
ProvenanceCol() = ProvenanceCol(IdDict{Int32,Any}())
