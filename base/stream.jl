...
function closewrite(s::LibuvStream; cancel::CancelTokenArg=DEFAULT_CANCEL)
    ...
    preserve_handle(ct)
    sigatomic_begin()
    # Register as a cancellable stream wait. A shutdown request itself cannot
    # be cancelled, so an interrupted wait detaches it (its completion
    # callback frees it).
...
function _uv_write_wait(s::LibuvStream, p::Ptr{UInt8}, n::UInt,
    ...
    if src !== nothing && !register_cancellation!(src, w)
        ...
        # caller's post-write cancellation point delivers the request.
...
function _uv_write_cancelled_teardown!(s::LibuvStream, w::WaitEntry, uvw::Ptr{Cvoid},
    ...
    ccall(:uv_cancel, Cint, (Ptr{Cvoid},), uvw) # ignore any errors
    detach = true
    if err isa CancellationRequest && severity(err) < severity(CANCEL_REQUEST_ABANDON_EXTERNAL) &&
