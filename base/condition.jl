...
# the owner's cached entry becomes reusable right away). Preconditions: the
# caller holds the entry's claim and the entry's owner is quiescent (claimed
# but not yet rescheduled). The only possible concurrent mutator is then a
# lock-holding notifier popping the entry, so `w.queue` can only transition
# to `nothing`: the racy read below is made safe by revalidating (via the
# witness check inside `list_deletefirst!`) after the lock is acquired.
# Uses `trylock` because the interrupt path must never block; on failure the
# entry is simply collected lazily (by the owner's own wait cleanup or by a
# later notify). Returns whether the entry is known to be unlinked.
#
...
function wait(c::GenericCondition, tok::MaybeToken; first::Bool=false, waitee=c,
    ...
    # already unlinked by the notifier; the source registration is ours to drop
    relockall(c.lock, token)
    src === nothing || unregister_cancellation!(src, w)
    list_deletefirst!(ILLRef(c.waitq, waitee), w)
    return ret
